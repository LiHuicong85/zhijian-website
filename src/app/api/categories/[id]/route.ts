...
export async function PUT(
  ...
  try {
    ...
    const result = await restRequest(
      `/${tableName}?id=eq.${id}`,
      'PATCH',
      { ...(name ? { name } : {}), ...(sort_order !== undefined ? { sort_order } : {}) }
    );

    if (result.status !== 200 && result.status !== 204) {
      console.error('[Categories API] PUT error:', result.status, result.body);
      return NextResponse.json({ success: false, error: `更新分类失败: ${result.status}` }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    const result = await restRequest(`/${tableName}?id=eq.${id}`, 'DELETE');

    if (result.status !== 200 && result.status !== 204) {
      console.error('[Categories API] DELETE error:', result.status, result.body);
      return NextResponse.json({ success: false, error: `删除分类失败: ${result.status}` }, { status: 500 });
    }
